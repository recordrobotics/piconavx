...
board_state = {}
board_id = {}
health = {"mem_used": 0, "mem_total": 0, "core_temp": 0.0}
...
async def sensor_poll():
    ...
    while True:
        ...
        try:
            ...
            ahrspos_update = NAV.ahrspos_update.copy()
            if NAV.IsDisplacementSupported():
                ...
                ahrs_update["quat_w"] = ahrspos_update["quat_w"]
                ahrs_update["quat_x"] = ahrspos_update["quat_x"]
                ahrs_update["quat_y"] = ahrspos_update["quat_y"]
                ahrs_update["quat_z"] = ahrspos_update["quat_z"]
                ahrs_update["barometric_pressure"] = ahrspos_update[
                    "barometric_pressure"
                ]
                ahrs_update["baro_temp"] = ahrspos_update["baro_temp"]
                ...
                ahrs_update["cal_mag_x"] = NAV.raw_data_update["mag_x"]
                ahrs_update["cal_mag_y"] = NAV.raw_data_update["mag_y"]
                ahrs_update["cal_mag_z"] = NAV.raw_data_update["mag_z"]
                ahrs_update["mag_field_norm_ratio"] = 0
                ahrs_update["mag_field_norm_scalar"] = 0
                ahrs_update["raw_mag_x"] = NAV.raw_data_update["mag_x"]
                ahrs_update["raw_mag_y"] = NAV.raw_data_update["mag_y"]
                ahrs_update["raw_mag_z"] = NAV.raw_data_update["mag_z"]
            else:
                ahrs_update = NAV.ahrs_update.copy()
            raw_data_update = NAV.raw_data_update.copy()
            ...
            if writer is None or reader is None:
                                ...
                                while reader is not None:
                                    try:
                                        ...
                                        if command_type == COMMAND_TYPE_SET_DATA:
                                            updateDataType_new = parse_set_data_command(
                                                host_line
                                            )
                                            if updateDataType_new != updateDataType:
                                                if (
                                                    ...
                                                    updateDataType == SET_DATA_TYPE_FEED
                                                ):
                                                    feed_A.disable()
                                                    feed_B.disable()
                                            updateDataType = updateDataType_new
                                        elif (
                                            command_type == COMMAND_TYPE_REQUEST_HEALTH
                                        ):
                                            health_requested = True
                                        elif (
                                            command_type
                                            == COMMAND_TYPE_REQUEST_BOARDSTATE
                                        ):
                                            boardstate_requested = True
                                        elif (
                                            command_type == COMMAND_TYPE_REQUEST_BOARDID
                                        ):
                                            ...
                                            command_type
                                            == COMMAND_TYPE_ZERO_DISPLACEMENT
                                        ):
                                            zerodisp_requested = True
                                        elif (
                                            command_type
                                            == COMMAND_TYPE_SET_FEED_OVERFLOW
                                        ):
                                            feed_A.feed_overflow = (
                                                feed_B.feed_overflow
                                            ) = parse_set_feed_overflow_command(
                                                host_line
                                            )
                                    except OSError as e:
                                        if (
                                            e.errno is errno.ECONNRESET
                                            or e.errno is errno.ECONNABORTED
                                            or e.errno is errno.ECONNREFUSED
                                        ):
                                            break
                                        else:
                                            raise e
                ...
                try:
                    if updateDataType == SET_DATA_TYPE_RAW:
                        ...
                        updateDataType == SET_DATA_TYPE_FEED
                        and time.ticks_diff(time.ticks_ms(), lastFeedSendTime) > 3000
                    ):
